...
const FlashcardView: React.FC<FlashcardViewProps> = ({ user, table, excludeMastered = false, onBack, onUpdateProgress, onAwardTokens }) => {
  ...
  const [actionHistory, setActionHistory] = useState<Array<{ type: 'known' | 'learning'; entryId: string }>>([]);
  ...
  const getCardWrapperStyle = (): React.CSSProperties => {
    if (transitionState === 'exiting') {
      const x = transitionDir === 'left' ? '-120%' : '120%';
      return {
        transform: `translateX(${x}) scale(0.95)`,
        opacity: 0,
  ...
  const handleAssessment = (known: boolean) => {
    ...
    setActionHistory(prev => [...prev, { type: known ? 'known' : 'learning', entryId: currentEntry.id }]);
  ...
  const handleGoBack = () => {
    if (transitionState !== 'idle' || actionHistory.length === 0) return;
    ...
    setTimeout(() => {
      if (lastAction.type === 'known') {
        onUpdateProgress(lastAction.entryId, -20);
        setKnownCount(prev => Math.max(0, prev - 1));
        setSessionTokens(prev => Math.max(0, prev - 5));
      } else {
        onUpdateProgress(lastAction.entryId, 35);
