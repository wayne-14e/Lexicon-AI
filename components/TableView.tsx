...
const TableView: React.FC<TableViewProps> = ({ table, onBack, onDelete, onStudy, onLearnContext, onMatchingGame, onUpdateTable }) => {
  ...
  const handleExport = async () => {
    ...
    // Inline logo so the exported HTML works when opened locally
    // (absolute `/logo.svg` works on the dev server, but not inside a downloaded file).
    let logoSrc = '/logo.svg';
    try {
      // Some TS setups don't include `ImportMetaEnv` typing; cast to avoid type errors.
      const baseUrl =
        ((import.meta as { env?: { BASE_URL?: string } }).env?.BASE_URL as string | undefined) ?? '/';
      ...
      console.warn('Failed to inline logo for export:', e);
    }
      ...
      <body class="font-sans text-gray-900 antialiased p-4 md:p-8">
        <div class="max-w-[95vw] mx-auto">
          ...
          <div class="bg-white p-6 md:p-10 border border-blue-100 rounded-xl shadow-xl shadow-blue-900/5 print:shadow-none print:border-none print:p-0 print:bg-white">
            ...
            ${table.description ? `<p class="text-lg italic font-display text-gray-600 mb-10 leading-relaxed max-w-4xl border-l-4 border-blue-100 pl-6">"${table.description}"</p>` : ''}
